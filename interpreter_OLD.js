...
function parseOperation(lines){
    ...
    const ref2 = lastWord(lines[lines.length-2]);
    const x = lastWord(lines[lines.length-1]);
    const operation = mapInterveningLineNumberToOperation(lines.length-3)
    const operationObj = new AST.Operation(new AST.Ref(ref1), new AST.Ref(ref2), operation);
    return new AST.Assign(x, operationObj);
}
